...
def f(q_f,q_b,last_name,next_name,init_random=random.randint(0,7)):
    ...
    while q_f.get()==1:
        ...
        print("now step %d"%step,"| receive %s data and computing...."% last_name,"send data to %s"%next_name,"||cost time %d s"%end_time)
...
def ring_reduce(num_node):
    ...
    for i in range(num_node):
        ...
        p=Process(target=f,args=(Q_L[last_index],Q_L[next_index],last_name,next_name))
        P_L.append(p)
